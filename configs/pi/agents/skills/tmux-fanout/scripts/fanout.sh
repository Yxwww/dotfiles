...
# jobids are generated by new_jobid() as job-YYYYMMDD-HHMMSS-aaaa. Validate
# any user-supplied jobid (status/logs/kill) against this shape before it flows
# into jobdir() -> rm -rf, to prevent path traversal (e.g. `kill ../..`).
...
cmd_status() {
  ...
  valid_jobid "$1" || die "invalid jobid: $1"
