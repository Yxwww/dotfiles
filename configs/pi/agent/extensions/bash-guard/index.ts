...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag("bash-guard-enabled", {
		description: "Start the session with bash-guard enabled (interactive prompting). Default is off; the catastrophic hard-block floor still applies when off.",
		type: "boolean",
		default: false,
	});

	// Session-local state. Intentionally not persisted across reloads or restarts.
	// Default is OFF (disabled); opt in to interactive prompting with --bash-guard-enabled.
	let disabled = initialDisabled(false);

	pi.on("session_start", async (event, ctx) => {
		if (event.reason !== "startup") return;
		disabled = initialDisabled(pi.getFlag("--bash-guard-enabled") === true);
		if (disabled) {
			const { theme } = ctx.ui;
	...
	pi.registerCommand("bash-guard", {
		description: "Toggle bash-guard between disabled (default) and enabled (interactive) for this session.",
