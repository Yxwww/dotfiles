...
### Main session (`PI_SUBAGENT_DEPTH` = 0 or unset) — off by default

Main sessions default to **off (autonomous)**: no interactive prompts, but a hard-block floor
still applies for catastrophic operations (the subagent table below, minus routine
`git commit`/`git pull`/`git push`). Opt in to interactive prompting with `--bash-guard-enabled`,
or toggle mid-session with `/bash-guard`.

**When enabled** (interactive prompt):
...
- `--bash-guard-enabled`: main-session flag to start with the prompt ON (interactive). Default
  is off (autonomous) with the catastrophic hard-block floor still active.
